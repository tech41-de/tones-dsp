...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_tonesDSP_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_tonesDSP_rustbuffer_free(self, $0) }
    }
}
...
open class Greeter:
    ...
    public func uniffiClonePointer() -> UnsafeMutableRawPointer {
        return try! rustCall { uniffi_tonesDSP_fn_clone_greeter(self.pointer, $0) }
    }
public convenience init(name: String) {
    let pointer =
        try! rustCall() {
    uniffi_tonesDSP_fn_constructor_greeter_new(
    ...
    deinit {
        ...
        try! rustCall { uniffi_tonesDSP_fn_free_greeter(pointer, $0) }
    }
...
open func greet() -> String {
    ...
    uniffi_tonesDSP_fn_method_greeter_greet(self.uniffiClonePointer(),$0
    )
})
}
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_tonesDSP_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_tonesDSP_checksum_func_add() != 61398) {
